    ...
    for cosmo in range(start_cosmo, start_cosmo + n_cosmo):
        ...
        for phase in range(start_phase, start_phase + n_phase):
            ...
            for hod in range(start_hod, start_hod + n_hod):
                ...
                if save_nbar:
                    nbar = len(x) / boxsize**3
                    N_gal = len(x)
                    output_dir = Path(f'/global/homes/e/epaillas/carolscratch/ds_boss/nbar/HOD/{dataset}/',
                        f'AbacusSummit_base_c{cosmo:03}_ph{phase:03}/z0.500/')
                    Path(output_dir).mkdir(parents=True, exist_ok=True)
                    output_fn = Path(
                        output_dir,
                        f'nbar_c{cosmo:03}_ph{phase:03}_hod{hod}.npy'
                    )
                    cout = {'nbar': nbar, 'N_gal': N_gal}
                ...
                for los in ['x', 'y', 'z']:
                    ...
                    print(f'cosmo {cosmo}, hod {hod}, los {los}')
                ...
                output_dir = Path(f'/global/homes/e/epaillas/carolscratch/ds_boss/tpcf_multipoles/HOD/{dataset}/',
                    f'AbacusSummit_base_c{cosmo:03}_ph{phase:03}/z0.500/')
                Path(output_dir).mkdir(parents=True, exist_ok=True)
                output_fn = Path(
                    output_dir,
                    f'tpcf_multipoles_c{cosmo:03}_ph{phase:03}_hod{hod}.npy'
                )
                np.save(output_fn, cout)
